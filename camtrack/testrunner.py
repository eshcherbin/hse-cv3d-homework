...
def _load_or_calculate_corners(grayscale_seq, test_name,
    ...
    if corner_storage:
        ...
        return without_short_tracks(corner_storage, min_len=min_track_len)
    try:
...
@click.option('--min_track_len', '-l', type=click.IntRange(min=0),
              default=10)
def cli(config_path, output_dir, corners_dir, corners_config_file, min_track_len):
    config = read_config(config_path)
    corners_config = yaml.load(corners_config_file)
    run_tests(config, output_dir, corners_dir, corners_config, min_track_len)
